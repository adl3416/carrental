...
public class AuthTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (jwt != null && jwtUtils.validateJwtToken(jwt)) {
                ...
                SecurityContextHolder.getContext().setAuthentication(authentication); //buraya Authentication yi ekledik
            }
        }catch (Exception e){
        ...
        filterChain.doFilter(request, response); //butunyaptiklarimizi  buraya ekledik
    }
    ...
    private String parseJwt(HttpServletRequest request) {
        ...
        if (StringUtils.hasText(headerAuth) && headerAuth.startsWith("Bearer ")){
            return headerAuth.substring(7);   // 7 ve 7 den sonrasini aldik
        }
        return null;
    }
}
